...
describe("syncContent", () => {
  ...
  it("prefixes every page with its title and description", async () => {
    ...
    assert.match(overview, /本插件需要 OpenClaw `>=2026\.6\.1`/);
    ...
    assert.match(chinesePrompt.value, /npm 非交互安装加 `--force`/);
    ...
    assert.match(english, /This plugin requires OpenClaw `>=2026\.6\.1`/);
    ...
    assert.match(englishPrompt.value, /Add `--force` for a noninteractive npm install/);
