    ...
    if (shouldFetch) {
      ...
      try {
        ...
        if (resp.ret === 0) {
          ...
          this.log(`[weixin] config ${entry?.everSucceeded ? "refreshed" : "cached"} for ${redactToken(userId)}`);
          fetchOk = true;
        }
      } catch (err) {
        this.log(`[weixin] getConfig failed for ${redactToken(userId)} (ignored): ${redactError(err)}`);
      }
      if (!fetchOk) {
