...
- **发布准备：** 更新 `3.2.2` 发布元数据，不改变运行时行为、宿主兼容性、依赖或插件/频道 ID。

## [3.2.1] - 2026-09-18

### 变更

- **OpenClaw 开发 SDK：** 固定为 `2026.9.4` 并同步构建元数据，推荐开发环境更新为
  Node.js `24.16.0`。CI 保留旧宿主和 Node.js `22.22.3` 覆盖；插件的最低宿主和
  Node.js 运行时要求不变。
...
- **维护报告预览：** 新增手动触发的 gh-aw 维护摘要，仅以 staged 模式预览结果，并分别
  限制主任务和威胁检测的推理额度；不自动创建 issue、PR 或执行发布。

### 变更

- **入站宿主编排：** 使用共用公开上下文 builder，按运行时能力选择路由型或旧版公开分派，
  同时保留 OpenClaw `2026.6.1` 下限、账号隔离和独立审批通道。分派失败不会改走另一
  契约重试。
...
- **插件图标元数据：** 移除 ClawHub 针对 OpenClaw `2026.9.4` 校验时报告的顶层
  manifest `icon` URL，将现有品牌图案作为 `assets/icon.png` 打包到所有 registry
  产物中。插件/频道 ID 和最低宿主版本保持不变；图标显示取决于宿主是否支持包内图标。
- **回复 Hook：** 现代入站回复和宿主管理的直接文本/媒体发送由宿主统一执行 hook，避免
  重复修改内容或重复观察发送结果。旧版入站回复与独立调试消息保留本地 hook；成功回复
  返回既有客户端传输 ID。
- **登录重载：** 改为在宿主的当前源配置上只更新频道时间戳，并显式采用宿主的自动重载
  策略，避免旧运行时快照覆盖其他配置。配置写入失败不会回滚已保存的登录凭据。
