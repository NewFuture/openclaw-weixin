...
- **入站 getUpdates 双投递：** ordinary / approval 两条 admission 车道在处理前通过
  OpenClaw `createClaimableDedupe`（`pluginId: openclaw-weixin`，按账号 namespace）
  认领稳定去重键（`message_id` → `client_id` → `seq` → 正文指纹），成功后写入
  24 小时 plugin-state 墓碑，避免 iLink 长轮询至少一次投递（约 1 秒重放）以及
  长任务卡住后的更长窗口重投把同一条消息跑两遍 AI，并在进程重启后仍生效。失败
  与 abort 会 release 以便重试。该窗口是**重放去重 / 墓碑窗口**，不会吞掉用户
  故意再发且带有新 `message_id` 的消息。存在传输层 id 时 `MessageSid` 使用同一
  稳定键。移植自
