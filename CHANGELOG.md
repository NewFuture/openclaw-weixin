...
- **GitHub Packages 补发：** 镜像缺少中间版本时不再阻塞当前精确版本；流程会记录警告、
  拒绝将 `latest` 回退到较旧版本、在发布前再次检查精确目标和 `latest`，并继续对非 404
  查询错误显式失败。新增仅限 `main` 上不可变 `v3.1.3` tag 的一次性手动补发流程，
  无需回填 3.1.2，也不会移动 tag 或授予 OIDC 权限。
